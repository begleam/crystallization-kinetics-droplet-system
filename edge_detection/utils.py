...
class CrystalTestDataset(Dataset):
    def __init__(self,
                 img_path_list,
                 img_height,
                 img_width,
    ...
    def __getitem__(self, idx):
        ...
        file_name = image_path.split('/')[-1].split('.')[0] + ".png"

        # load data
        image = cv2.imread(image_path, cv2.IMREAD_COLOR)

        im_shape = [image.shape[0], image.shape[1]]
        image, label = self.transform(img=image, gt=None)
...
class PHATestDataset(Dataset):
    ...
    def transform(self, img, gt):
        if self.image_preprocess_func is not None:
            img = self.image_preprocess_func(img)
        img_height = self.img_height
        img_width = self.img_width
        # print(f"actual size: {img.shape}, target size: {( img_height,img_width,)}")
        img = cv2.resize(img, (img_width,img_height))
...
def get_boundary_points(tensor, img_shape, save_dir, file_names, return_images=False, mode='fused', show_progress=False):
    # "mode" should be 'fused' or 'avg'
    output_dir = os.path.join(save_dir, mode)
    ...
    for i_shape, file_name in zip(image_shape, file_names):
        ...
        if mode=='avg':
            ...
            result_img = np.array(preds, dtype=np.float32)
            result_img = np.uint8(np.mean(result_img, axis=0))
        elif mode=='all':
            preds = []
            for i in range(tmp.shape[0]):
                tmp_img = tmp[i]
            ...
            # Get the mean prediction of all the 7 outputs
            result_img = np.array(preds, dtype=np.float32)
            result_img = np.uint8(255 - np.clip(np.sum(255 - result_img, axis=0), 0, 255))
        elif mode=='fused':
            tmp_img = tmp[6]
            tmp_img = np.uint8(image_normalization(tmp_img))
            tmp_img = cv2.bitwise_not(tmp_img)
        ...
        if show_progress:
            print(f"Result image saved at {output_file_name}")
        if return_images:
            result_images.append(result_img)
    return result_images

def get_edges(model, dataloader, device, save_dir, show_progress=False):
    model.eval()
    with torch.no_grad():
        for batch_id, sample_batched in tqdm(enumerate(dataloader), total=len(dataloader), disable=not show_progress, desc=f"{save_dir}"):
